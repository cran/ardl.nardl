...
{
  if("(Intercept)" %in% names(nardl_fit_case_$coefficients) == TRUE){
    keep_val <- c(1)
  }
  else{
    keep_val <- NULL
...
{
  if("trend" %in% names(nardl_fit_case_$coefficients) == TRUE){
    keep_val <- c(keep_val,length(nardl_fit_case_$coefficients))
  }
  else{
    keep_val <- keep_val
  }
...

{
  if(don > 0 & dop > 0 ){
    ...
    if(length(d_pos) > 1){
      dddneg <- paste(d_neg, " + ")
      dddpos <- paste(d_pos," = 0")
      plus_ <- dddpos[[length(dddpos)]]
      plus_ <- gsub(x = plus_, pattern = "[=0]| ", replacement = "")
      plus_ <- paste(plus_,' + ')
      dddpos <- str_flatten(c(plus_, dddpos[-length(dddpos)]))
      pos_neg <-  str_flatten(c(dddneg,dddpos))
      wld_test <- linearHypothesis(ecm_gets_fit, hypothesis.matrix = pos_neg, verbose = F,test = "F")
      sr_asym_test <- cbind(Fstat = wld_test$F[2], Pval = wld_test$`Pr(>F)`[2])
      rownames(sr_asym_test) <- decomp
      sr_asym_test
